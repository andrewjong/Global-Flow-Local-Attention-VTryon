...
class DanceDataset(AnimationDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument("--use_clean_pose", dest="clean_pose", action="store_true", help="Use the video2d folder generated by the Motion Extraction Net")
        parser.add_argument("--no_clean_pose", dest="clean_pose", action="store_false", help="Just use the noisy alphapose, no clean pose")
        parser.set_defaults(clean_pose=True)
    ...
    def get_paths(self, opt):
        ...
        maybe_clean_dir = "video2d" if opt.clean_pose else "alphapose"
        dir_B_clean = os.path.join(opt.dataroot, phase_dir, maybe_clean_dir)
    ...
    def __getitem__(self, index):
        ...
        for i in range(n_frames_total):
            ...
            skeleton = self.load_skeleton(B_path, is_clean_pose=self.opt.clean_pose)
            gen_skeleton = self.concat_frame(gen_skeleton, skeleton)

            if self.opt.use_kp:
                # load key point joints
                B_path = B_paths_clean[start_idx + i * t_step]
                kp = self.load_keypoint(B_path, is_clean_pose=self.opt.clean_pose)
